...
sigdatatypes.validatePower(P_in,'waterfill','PT',{'vector'});
...
if cond
    coder.internal.errorIf('phased:phased:invalidRowNumbers','NPOW',numel(P));
end
...
Pw = zeros(size(Nvec));
for m = 1:L
    rk = find(PtotRequired(m,:)<P(m),1,'last');
